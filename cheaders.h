/**
 * \file cheaders.h
 * \brief Prototypes for BPQ32 Node Functions
 */
...
/*! \brief My node callsign (ASCII) */
extern UCHAR MYNODECALL[];

/*! \brief My node callsign (ASCII -- NULL terminated) */
extern char NODECALLLOPPED[];

/*! \brief My NET/ROM callsign (ASCII) */
extern UCHAR MYNETROMCALL[];

/*! \brief My NET/ROM callsign (AX.25) */
extern UCHAR NETROMCALL[];
...
// IP, APRS use port config slots above the real port range
...
extern char UIUIDEST[MaxBPQPortNo + 1][11];			// Destination for Beacons
extern UCHAR FN[MaxBPQPortNo + 1][256];				// Filename
extern int Interval[MaxBPQPortNo + 1];				// Beacon Interval (minutes)
